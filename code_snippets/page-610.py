...
MODEL_PATH = 'fashion_mnist.parms'
...
model1 = Model()
model1.add(Layer_Dense(X.shape[1],128,weight_regularizer_l2=5e-4,bias_regularizer_l2=5e-4))
...
model1.finalize()
model1.train(X,y,validation_data=(X_test, y_test),epochs=10,batch_size=128,print_every=100)

model1.save_parameters(MODEL_PATH)

model2 = Model()
model2.add(Layer_Dense(X.shape[1],128,weight_regularizer_l2=5e-4,bias_regularizer_l2=5e-4))
